...
char  commands[RSIZ][LSIZ];
// static const char *commands[] = {"login", "password", "passwor?d"};
...
bool read_file() {
  ...
  while (fgets(commands[i], LSIZ, fptr)) {
...
size_t afl_custom_fuzz(my_mutator_t *data, uint8_t *buf, size_t buf_size,
  ...
  if (debug == true) {
    ...
    // mutated_out,strlen(mutated_out));
    printf("Before %s\n", mutated_out);
  }

  // Randomly select a command string to add as a header to the packet
  char *to_send = commands[rand() % n_commands];
  int   len_command = strlen(commands[rand() % n_commands]);
